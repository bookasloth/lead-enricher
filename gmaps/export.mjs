...
export const SALES_COLS = ['name', 'category', 'locality', 'address', 'phone', 'whatsapp', 'email',
  'website', 'booking_link', 'rating', 'review_count', 'has_website', 'has_phone', 'has_email',
  'has_social', 'has_booking', 'has_whatsapp', 'branch_count', 'score', 'score_reasons', 'maps_url', 'found_count'];
...
export function flatten(lead, includeRaw = false) {
  ...
  const row = {};
  for (const c of SALES_COLS) row[c] = c === 'score_reasons' ? reasons : (lead[c] ?? '');
